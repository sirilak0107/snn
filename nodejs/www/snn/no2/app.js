...
async function loadZoneData(dateStr) {
    try {
        ...
        const [urbanRes, suburbanRes, ruralRes] = await Promise.all([
            fetch(`/snn/api/no2_urban/${dateStr}`),
            fetch(`/snn/api/no2_suburban/${dateStr}`),
            fetch(`/snn/api/no2_rural/${dateStr}`)
        ]);
            ...
            if (r.geom) {
                ...
                L.geoJSON(feature, {
                    ...
                    onEachFeature: function (f, layer) {
                        const val = parseFloat(f.properties.no2);
                        const text = `
                          <b>NO₂</b>: ${val.toFixed(6)} ${NO2_UNITS}
                        `;
...
function setPanel(mode) {
    ...
    if (mode === 'A') {
        title.textContent = 'NO₂ Map · From GEE';
        desc.textContent = 'การแสดงผลปริมาณ NO₂ จากข้อมูลจริงของ GEE';
    } else {
        title.textContent = 'NO₂ Map · From GEE';
        desc.textContent = 'การแสดงผลปริมาณ NO₂ จากข้อมูลจริงของ GEE';
    }
...
function setActive(next) {
    ...
    if (next === 'A') {
        ...
        setPanel('A'); showToast('สลับเป็น แผนที่ A (NO₂)');
    } else {
        if (map.hasLayer(tilesLight)) map.removeLayer(tilesLight);
        tilesDark.addTo(map);
        setPanel('B'); showToast('สลับเป็น แผนที่ B (CO)');
    }
}
